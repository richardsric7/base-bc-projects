...
| `RELAYER_KEY_SALT` / `RELAYER_POOL_SIZE` | Derives the pool of relayer addresses that submit every real Safe `execTransaction` call once a shared-access group's approval threshold is met (PLAN.md §13.10 Phase 4/§13.12) - fund every one of `RELAYER_POOL_SIZE` derived addresses with ETH (logged at boot). Same harmless-rotation property as `SAFE_DEPLOYER_KEY_SALT` - a relayer never has authority over a Safe, only gas to spend submitting a call someone else already fully approved. |
