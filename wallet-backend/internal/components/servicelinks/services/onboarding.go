...
// verified SIWE session: a partner-onboarded user's wallet is provisioned
// and held by the partner's own embedded-wallet infrastructure, not signed
// into directly against this API, so there is no SIWE session to prove
...
// PLAN.md §4.11's "what ports directly" note on onboarding.
